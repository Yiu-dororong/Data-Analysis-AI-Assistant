...
def create_dynamic_enrichment_model(column_names: List[str]):
    ...
    class Sufficiency(BaseModel):
        ...
        missing_info_reason: Optional[str] = Field(description="Why is context lacking?")
...
def enrichment_node(state: AgentState):
    ...
    # 2. Bind the structured output to the LLM
    # This tells the LLM (Gemma 3) to follow the Pydantic schema exactly
    structured_llm = llm.with_structured_output(EnrichmentModel)
    
    logger.info("Invoking LLM for data enrichment and context generation.")
    result = structured_llm.invoke(prompts.enrichment_prompt_template.format(
...
def create_feature_engineering_model(column_names: List[str]):
    ...
    class FERecipe(BaseModel):
        ...
        new_name: str
        numerator: Optional[ColumnsEnum] 
        denominator: Optional[ColumnsEnum] 
        
        minuend: Optional[ColumnsEnum] 
        subtrahend: Optional[ColumnsEnum]

        multiplicand: Optional[ColumnsEnum] 
        multiplier: Optional[ColumnsEnum]  
        
        addend_1: Optional[ColumnsEnum] 
        addend_2: Optional[ColumnsEnum]

        time_col: Optional[ColumnsEnum]
...
def feature_engineering_node(state: AgentState):
    ...
    result = llm.with_structured_output(FEModel).invoke(prompts.feature_engineering_prompt_template.format(
        enriched_context = state['enriched_context'],
        primary_metric = state['primary_metric'],
        secondary_metrics = state['secondary_metrics'],
        time_metadata = time_metadata,
        columns_names = state['column_names'],
        sample_values = state['sample_values'],
        data_profile=state['data_profile'],
    ))

    logger.info(f"LLM feature engineering plan received. Needed: {result.needed}")
    ...
    for action in result.actions:
        ...
        if action.type == "division" and action.numerator and action.denominator:
            df[action.new_name] = df[action.numerator.value] / df[action.denominator.value].replace(0, np.nan)
            ...
            if pd.api.types.is_datetime64_any_dtype(df[action.minuend.value]) and pd.api.types.is_datetime64_any_dtype(df[action.subtrahend.value]):
                df[action.new_name] = (df[action.minuend.value] - df[action.subtrahend.value]).dt.days
            ...
            df[action.new_name] = df[action.multiplicand.value] * df[action.multiplier.value]
            state['secondary_metrics'].append(action.new_name)
    ...
    # Update after new columns are made
    state["column_names"]=list(df.columns)
    state["sample_values"]=df.sample(5,ignore_index=True)
...
def create_planning_model(column_names: List[str]):
    ...
    class ChartSpec(BaseModel):
        ...
        x: ColumnsEnum 
        y: Optional[ColumnsEnum] 
        color_by: Optional[ColumnsEnum]
...
def visualisation_planning_node(state: AgentState):
    ...
    structured_llm = llm.with_structured_output(PlanningModel)
    
    logger.info("Invoking LLM for visualisation plan.")
    result = structured_llm.invoke(prompts.visualisation_plan_prompt_template.format(
        domain = state['domain'],
        enriched_context = state['enriched_context'],
        primary_metric = state['primary_metric'],
        secondary_metrics = state['secondary_metrics'],
    ...
    #Categorize and remove duplicate   
    ...
    for chart in result.charts:
        x = chart.x.value
        y = chart.y.value if chart.y else None
...
def create_visionary_analyst_model(column_names: List[str]):
    ...
    class ChartInsight(BaseModel):
        ...
        suggested_tweak: Optional[Literal[ 
            ...
            "facet_grid",
            "color_by_segment",
        ]] = Field(description="The specific technical fix required")
        quality_score: int = Field(ge=1, le=10, description="Visual clarity score (10 is the best)")
        facet_by: Optional[ColumnsEnum] = Field(description="Column to split the chart into a grid (e.g. 'Region')")
        color_by: Optional[ColumnsEnum] = Field(description="Column to color the chart (e.g. 'Category')")
...
def visionary_analyst_node(state: AgentState):
    ...
    structured_llm = llm.with_structured_output(VisionAuditModel)
    ...
    for i, batch_indices in enumerate(batches):
        ...
        try:
            # Get the PNG buffers for this specific batch
            ...
            # We convert the Pydantic object to a dict to store in the State (Optional)
                all_descriptions[idx] = insight_obj.dict()
                if type(chart_specs[0]) != tuple:
                    chart_spec = chart_specs[idx]
                else:
                    chart_spec = chart_specs[idx][0]
                ...
                if insight_obj.is_spurious:
                    ...
                    if (insight_obj.suggested_tweak == "color_by_segment") and (insight_obj.color_by in (chart_spec.x, chart_spec.y)):
                        drop_insight.append(insight_obj)
                        continue
                    # Not a valid facet grid (Cannot find the column)
...
def executive_summary_node(state: AgentState):
    ...
    response = llm.with_structured_output(ExecutiveBriefModel).invoke(prompt)
